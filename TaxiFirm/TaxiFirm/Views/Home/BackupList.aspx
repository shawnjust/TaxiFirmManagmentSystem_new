...
<%@ Import Namespace="TaxiFirm.Models.Backup"  %>
<%@ Import Namespace="TaxiFirm.Models" %>

<asp:Content ID="aboutTitle" ContentPlaceHolderID="TitleContent" runat="server">信息管理</asp:Content>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        function () {
            ...
            form1.action = "/Home/BackupList?type=search&NameID=" + name + "&page=1";
            form1.submit();
        ...
        function () {
            ...
            if (type == "Prev") {
                if (CurrentPage <= 1) {
                    ...
                    if ("common" == type) {
                        form1.action = "/Home/BackupList?type=common&page=" + (CurrentPage - 1);
                    } else if ("search" == type) {

                        var name = document.getElementById("name_id").value;
                        form1.action = "/Home/BackupList?type=search&NameID=" + name + "&page=" + (CurrentPage - 1);
                    }
                    form1.submit();
...
<% List<Backup> backups = (List<Backup>)ViewData["backups"];
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td height="24" class="CenterUp"><table width="100%" border="0" cellspacing="0" cellpadding="0">
     <tr>
      <td width="77" height="24" align="center" valign="middle" class="UpSelect1" style="color:#06F;">历史备份</td>
        <td width="77" height="24" align="center" valign="middle" class="UpSelect1" style="color:#06F;background-image:url(../../Content/picture/BackControl/CenterUp2Select.png);"><a href="/Home/Backup">我要备份</a></td>
        <td height="24">&nbsp;</td>
                ...
                <%  int current = page.CurrentPage - 3;
                    for (int i = 0; i < page.PageWidth; i++)
                    {
                        ...
                        if (current == page.CurrentPage)
                        {
                            ...
                            {%>
                      <li> <a href="/Home/BackupList?type=common&page=<%:current%>" style="background-color:Orange"><%:current%></a> </li>
                      <%}
                            else if ("search".Equals(type))
                            {%>
                            <li> <a href="/Home/BackupList?type=search&page=<%:current%>&NameID=<%:ViewData["NameID"] %>" style="background-color:Orange"><%:current%></a> </li>
                            <%
                            }
}
                            ...
                            if ("common".Equals(type))
                            {%>
                      <li> <a href="/Home/BackupList?type=common&page=<%:current%>"><%:current%></a> </li>
                      <%}
                            else if ("search".Equals(type))
                            {%>
                            <li> <a href="/Home/BackupList?type=search&page=<%:current%>&NameID=<%:ViewData["NameID"] %>"><%:current%></a> </li>
                            <%
                            }
                        else if (current < 1)
                        {
